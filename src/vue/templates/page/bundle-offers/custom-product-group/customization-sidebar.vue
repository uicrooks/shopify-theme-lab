<template>
    <div v-if="product.title" class="custom-product-group-customization-sidebar-component">
        <template #header>
             <span
                v-if="discountAmount"
                class="discount"
...
<template #default>
    ...
    <div v-if="product.variants" class="variants-box">
        <h3>Choose Your Scent Profile:</h3>
        <div class="variants">
            <div v-for="(variant, index) of product.variants" :key="`variant-${index}`" class="variant" @click="selectedVariantIndex = index">
                <div :class="{'selected': selectedVariantIndex === index}" class="variant-icon">
                    Icon
                </div>
    ...
    <div class="included-box">
        ...
        <div v-for="(category, categoryIndex) of includedListCategories" :key="`category-${categoryIndex}`" class="included-category">
            <h4>{{ category }}</h4>
            <custom-product-group-included-list v-for="(variant, variantIndex) of product.variants" :key="`variant-${variantIndex}`" :product-handles="getIncludedListForCategory(variant, category)" :hidden="variantIndex !== selectedVariantIndex" />
        </div>
    </div>
</template>

<template #footer>
    <squatch-button class="finish-button" @clicked="checkout">
        Finish
    </squatch-button>
</template>
   </div>
...
import CustomProductGroupIncludedList from "./included-list.vue";
...
export default {
    ...
    components: {
        CustomProductGroupIncludedList,
        SquatchButton
    }
    filters: {
        money
